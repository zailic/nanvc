...
@suite('VaultSystemWrappingClient unit test cases.')
export class VaultSystemWrappingClientUnitTests {
    ...
    @test('should look up wrapping properties for a given token')
    public async shouldLookUpWrappingPropertiesForAGivenTokenTest() {
    ...
    @test('should surface lookup errors from Vault')
    public async shouldSurfaceLookupErrorsFromVaultTest() {
        ...
        const postStub = this.sandbox
            .stub(RawVaultClient.prototype, 'post')
            .onFirstCall()
            .returns(resultOf(err(clientError)));
        postStub.onSecondCall().returns(
            resultOf(ok({})), // simulate invalid response without "data" property
        );
        const client = new VaultSystemWrappingClient(new RawVaultClient());

        await assert.rejects(client.lookup('s.abc123').unwrap(), (error: unknown) => {
    ...
    @test('should wrap data with the given TTL')
    public async shouldWrapDataWithTheGivenTtlTest() {
        const wrapInfo = {
            token: 's.wrapped',
        ...
        const postStub = this.sandbox
            .stub(RawVaultClient.prototype, 'post')
            .returns(resultOf(ok({ wrap_info: wrapInfo })));
        const client = new VaultSystemWrappingClient(new RawVaultClient());
    ...
    @test('should surface wrap errors from Vault')
    public async shouldSurfaceWrapErrorsFromVaultTest() {
        const clientError = new VaultClientError({
            code: 'HTTP_ERROR',
            message: 'Bad Request',
            status: 400,
        ...
        await assert.rejects(client.wrap({ foo: 'bar' }, '60s').unwrap(), (error: unknown) => {
            assert.equal(error, clientError);
            return true;
        });
    }

    @test('should unwrap a response-wrapped token')
    public async shouldUnwrapAResponseWrappedTokenTest() {
        const postStub = this.sandbox
            .stub(RawVaultClient.prototype, 'post')
            .returns(resultOf(ok({ data: { role_id: 'r1', secret_id: 's1' } })));
        const client = new VaultSystemWrappingClient(new RawVaultClient());
    ...
    @test('should surface unwrap errors from Vault')
    public async shouldSurfaceUnwrapErrorsFromVaultTest() {
        const clientError = new VaultClientError({
            code: 'HTTP_ERROR',
            message: 'Not Found',
            status: 404,
        ...
        await assert.rejects(client.unwrap('s.wrapping_token').unwrap(), (error: unknown) => {
            assert.equal(error, clientError);
            return true;
        });
    }

    @test('should rewrap a response-wrapped token')
    public async shouldRewrapAResponseWrappedTokenTest() {
        const wrapInfo = {
            token: 's.new_wrapped',
    ...
    @test('should surface rewrap errors from Vault')
    public async shouldSurfaceRewrapErrorsFromVaultTest() {
        const clientError = new VaultClientError({
            code: 'HTTP_ERROR',
            message: 'Forbidden',
            status: 403,
        ...
        await assert.rejects(client.rewrap('s.old_wrapping_token').unwrap(), (error: unknown) => {
            assert.equal(error, clientError);
            return true;
        });
    }
